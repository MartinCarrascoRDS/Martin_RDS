...
archivo_ventas = f'/Users/martincarrasco/Desktop/Martín_Carrasco/Análisis márgenes/10.Eliminar_sin_costos/{año}/{fecha}/Paso10_{cuenta_meli}_{fecha}_completo.xlsx'
hoja_ventas = 'Sheet1'
...
df.loc[mask1, 'Utilidad'] = (
    df.loc[mask1, 'Ingresos por productos (CLP) Neto'] - df.loc[mask1, 'Total costo']
)
...
df.loc[mask2, 'Margen'] = (
    ...
    df.loc[mask2, 'Ingresos por productos (CLP) Neto']
)
...
df["Estrategia Pricing"] = np.select(condiciones_estrategias, estrategias, default = 'Normal')
